...
void encode_packet(uint32_t size_byte, uint8_t *data, raw_packet_t *packet) {
  uint32_t i = 0;
  for (; i < PACKET_START_LEN; i++) {
    packet->bits[i] = PACKET_START[i];
  }
  for (uint32_t j = 0; j < PACKET_SIZE_LEN; j++) {
  ...
  for (uint32_t j = 0; j < PACKET_END_LEN; j++) {
...
typedef struct {
  ...
  uint32_t start, end; // data is in [start, end), circular buffer
} queue_t;
...
int decode_bit_at_pos(queue_t *buf, uint32_t pos) {
  int num_ones = 0;
  for (uint32_t j = 0; j < DECODE_SAMPLE_RATIO; j++) {
    num_ones +=
...
int decode_packet(raw_packet_t *received_bits, uint32_t *decoded_size_byte,
                  uint8_t decoded_data[PACKET_DATA_MAX_BYTE]) {
  static queue_t buf;

  // Append data to buffer
  for (uint32_t i = 0; i < received_bits->size; i++) {
  ...
  while (1) {
    ...
    // Find start sequence
    for (uint32_t i = 0; i < PACKET_START_LEN; i++) {
    ...
    int size = 0;
    for (uint32_t i = 0; i < PACKET_SIZE_LEN; i++) {
    ...
    *decoded_size_byte = size;
    ...
    // Find end sequence
    for (uint32_t i = 0; i < PACKET_END_LEN; i++) {
